...
def create_img_list(data_dir):
    ...
    #             subset, "{:05d}".format(folder_id), "{:04d}".format(img_id)
    #         )
...
def process_data(img_path, depth):
    ...
    with open(meta_path, "r") as f:
        ...
        for line in f:
            ...
            masks[:, :, i] = inst_mask
            coords[:, :, i, :] = np.multiply(
                coord_map, np.expand_dims(inst_mask, axis=-1)
            )
...
def annotate_camera_train(data_dir):
    ...
    camera_train = (
        open(os.path.join(data_dir, "CAMERA", "train_list_all.txt")).read().splitlines()
    )
    ...
    for img_path in tqdm(camera_train):
        ...
        all_exist = (
            os.path.exists(img_full_path + "_color.png")
            and os.path.exists(img_full_path + "_coord.png")
            and os.path.exists(img_full_path + "_depth.png")
            and os.path.exists(img_full_path + "_mask.png")
            and os.path.exists(img_full_path + "_meta.txt")
        )
        if not all_exist:
            continue
        ...
        scales, rotations, translations, error_messages, _ = align_nocs_to_depth(
            masks, coords, depth, intrinsics, instance_ids, img_path
        )
        if error_messages:
            continue
...
def annotate_real_train(data_dir):
    ...
    real_train = (
        open(os.path.join(data_dir, "Real/train_list_all.txt")).read().splitlines()
    )
    intrinsics = np.array(
    ...
    for img_path in tqdm(real_train):
        ...
        for i in range(num_insts):
            ...
            retval, rvec, tvec = cv2.solvePnP(
                coord_pts, img_pts, intrinsics, distCoeffs
            )
            assert retval
...
def annotate_test_data(data_dir):
    ...
    # camera_val = (
    #     open(os.path.join(data_dir, "CAMERA", "val_list_all.txt")).read().splitlines()
    # )
    real_test = (
        open(os.path.join(data_dir, "Real", "test_list_all.txt")).read().splitlines()
    )
    camera_intrinsics = np.array([[577.5, 0, 319.5], [0, 577.5, 239.5], [0, 0, 1]])
    real_intrinsics = np.array(
        [[591.0125, 0, 322.525], [0, 590.16775, 244.11084], [0, 0, 1]]
    )
    # compute model size
    model_file_path = [
    ...
    subset_meta = [
        # ("CAMERA", camera_val, camera_intrinsics, "val"),
    ...
    for source, img_list, intrinsics, subset in subset_meta:
        ...
        for img_path in tqdm(img_list):
            ...
            nocs_dir = os.path.join("results", "nocs_results")
            if source == "CAMERA":
                nocs_path = os.path.join(
                    nocs_dir,
                    "val",
                    "results_val_{}_{}.pkl".format(
            ...
            for i in range(num_insts):
                ...
                assert gt_match > -1, print(
                    img_path, instance_ids[i], "no match for instance"
                )
