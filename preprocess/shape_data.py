...
def save_nocs_model_to_file(obj_model_dir):
    ...
    special_cases = {
        "3a7439cfaa9af51faf1af397e14a566d": np.array([0.115, 0.0, 0.0]),
    ...
    for subset in ["train", "val"]:
        ...
        for synsetId in [
            ...
            for instance in inst_list:
                ...
                model_points = sample_points_from_mesh(
                    path_to_mesh_model, 1024, fps=True, ratio=3
                )
                ...
                if synsetId == "03797390":
                    ...
                    if instance in special_cases.keys():
                        ...
                        shift_x = (
                            np.amin(model_points[:, 2]) - np.amax(model_points[:, 2])
                        ) / 2 - np.amin(model_points[:, 0])
        ...
        with open(
            os.path.join(obj_model_dir, "camera_{}.pkl".format(subset)), "wb"
        ) as f:
...
def save_model_to_hdf5(
    obj_model_dir, n_points, fps=False, include_distractors=False, with_normal=False
):
        ...
        for catId in range(1, 7):
            ...
            for instance in inst_list:
                ...
                if instance == "b9be7cfe653740eb7633a2dd89cec754":
                    continue
                model_points = sample_points_from_mesh(
                    path_to_mesh_model, n_points, with_normal, fps=fps, ratio=2
                )
                model_points = model_points * np.array([[1.0, 1.0, -1.0]])
                if catId == 6:
    ...
    for subset in ["real_train", "real_test"]:
        ...
        for inst_path in sorted(path_to_mesh_models):
            ...
            model_points = sample_points_from_mesh(
                inst_path, n_points, with_normal, fps=fps, ratio=2
            )
            bbox_file = inst_path.replace(".obj", ".txt")
            bbox_dims = np.loadtxt(bbox_file)
    ...
    train_dataset.create_dataset(
        "data", data=train_data, compression="gzip", dtype="float32"
    )
    train_dataset.create_dataset(
        "label", data=train_label, compression="gzip", dtype="uint8"
    )
    val_dataset = hfile.create_group("val")
    val_dataset.attrs.create("len", num_val_instances)
    val_dataset.create_dataset(
        "data", data=val_data, compression="gzip", dtype="float32"
    )
    val_dataset.create_dataset(
        "label", data=val_label, compression="gzip", dtype="uint8"
    )
    hfile.close()
