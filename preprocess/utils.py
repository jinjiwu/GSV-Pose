...
def compute_3d_IoU(
    sRT_1, sRT_2, size_1, size_2, class_name_1, class_name_2, handle_visibility
):
    ...
    if (class_name_1 in ["bottle", "bowl", "can"] and class_name_1 == class_name_2) or (
        ...
        for i in range(n):
            ...
            max_iou = max(
                max_iou, asymmetric_3d_iou(rotated_RT_1, sRT_2, size_1, size_2)
            )
    else:
...
def compute_mAP(
    ...
    iou_pred_matches_all = [
        np.zeros((num_iou_thres, 30000)) for _ in range(num_classes)
    ]
    ...
    pose_pred_matches_all = [
        np.zeros((num_degree_thres, num_shift_thres, 30000)) for _ in range(num_classes)
    ]
    ...
    for progress, result in enumerate(tqdm(pred_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            cls_pred_sRT = (
                pred_sRT[pred_class_ids == cls_id]
                if len(pred_class_ids)
                else np.zeros((0, 4, 4))
            )
            cls_pred_size = (
                pred_size[pred_class_ids == cls_id]
                if len(pred_class_ids)
                else np.zeros((0, 3))
            )
            cls_pred_scores = (
            ...
            if use_matches_for_pose:
                ...
                cls_pred_sRT = (
                    cls_pred_sRT[iou_thres_pred_match > -1]
                    if len(iou_thres_pred_match) > 0
                    else np.zeros((0, 4, 4))
                )
                cls_pred_scores = (
                ...
                cls_gt_sRT = (
                    cls_gt_sRT[iou_thres_gt_match > -1]
                    if len(iou_thres_gt_match) > 0
                    else np.zeros((0, 4, 4))
                )
                cls_gt_handle_visibility = (
            ...
            pose_cls_gt_match, pose_cls_pred_match = compute_RT_matches(
                RT_overlaps,
                cls_pred_class_ids,
                cls_gt_class_ids,
                degree_thres_list,
            ...
            pose_pred_matches_all[cls_id][:, :, pred_start:pred_end] = (
                pose_cls_pred_match
            )
            ...
            pose_pred_scores_all[cls_id][:, :, pred_start:pred_end] = (
                cls_pred_scores_tile
            )
    ...
    for cls_id in range(num_classes):
        ...
        iou_pred_scores_all[cls_id] = iou_pred_scores_all[cls_id][
            :, : iou_pred_count[cls_id]
        ]
        iou_gt_matches_all[cls_id] = iou_gt_matches_all[cls_id][
            :, : iou_gt_count[cls_id]
        ]
        # pose
        pose_pred_matches_all[cls_id] = pose_pred_matches_all[cls_id][
            :, :, : pose_pred_count[cls_id]
        ]
        pose_pred_scores_all[cls_id] = pose_pred_scores_all[cls_id][
            :, :, : pose_pred_count[cls_id]
        ]
        pose_gt_matches_all[cls_id] = pose_gt_matches_all[cls_id][
            :, :, : pose_gt_count[cls_id]
        ]

    # compute 3D IoU mAP
    for cls_id in range(1, num_classes):
        for s, iou_thres in enumerate(iou_thres_list):
            iou_aps[cls_id, s], iou_acc[cls_id, s] = compute_ap_and_acc(
    ...
    for i, degree_thres in enumerate(degree_thres_list):
        for j, shift_thres in enumerate(shift_thres_list):
            for cls_id in range(1, num_classes):
                ...
                pose_aps[cls_id, i, j], pose_acc[cls_id, i, j] = compute_ap_and_acc(
...
def plot_mAP(
    iou_aps, pose_aps, out_dir, iou_thres_list, degree_thres_list, shift_thres_list
):
