...
flags.DEFINE_string("dataset_dir", "data", "path to the dataset")
flags.DEFINE_string(
    "detection_dir", "data/segmentation_results", "path to detection results"
)
...
flags.DEFINE_float(
    "aug_bc_pro", 0.3, "box cage based augmentation, only valid for bowl, mug"
)
...
flags.DEFINE_integer(
    "face_recon_c",
    6 * 5,
    "for every point, we predict its distance and normal to each face",
)
...
)  # batchsize is 8, then 3000
#####################space is not enough, trade time for space####################
flags.DEFINE_integer(
    "accumulate", 1, ""
)  # the real batch size is batchsize x accumulate
...
flags.DEFINE_float("recon_v_w", 1.0, "voting loss weight")  # 1.0
...
flags.DEFINE_float(
    "geo_f_w", 0.1, "geo face loss, face must be consistent with the point cloud"
)
...
flags.DEFINE_float(
    "prop_sym_w",
    1.0,
    "importtannt for symmetric objects, can do point aug along reflection plane",
)
...
flags.DEFINE_string(
    "resume_model",
    "output/modelsave_all/gpv_pose_update_my.pth",
...
flags.DEFINE_boolean("cut", False, "cut point cloud, only valid when cut is True")
flags.DEFINE_float("cut_ratio", 0.0, "cut ratio, only valid when cut is True")
flags.DEFINE_integer("cut_repeat", 1, "cut repeat, only valid when cut is True")
flags.DEFINE_string("cut_method", "one_plane", "cut method")
flags.DEFINE_integer("cut_seed", 42, "random seed for cutting")

###################snapshot#################
flags.DEFINE_integer("test_epoch", 20, "save pcl when evaluation")
flags.DEFINE_integer("test_iter", 20, "inference without evaluation")
