...
sys.path.append(str(Path(__file__).resolve().parent.parent / "network"))
...
def train(argv):
    ...
    scheduler = build_lr_rate(
        optimizer, total_iters=train_steps * FLAGS.total_epoch // FLAGS.accumulate
    )
    ...
    for epoch in range(s_epoch, FLAGS.total_epoch):
        ...
        if len_last < train_size:
            ...
            if FLAGS.dataset == "CAMERA+Real":
                ...
                data_list = (
                    random.sample(camera_indices, 3 * n_repeat * real_len)
                    + real_indices * n_repeat
                )
        ...
        for i, data in enumerate(train_dataloader, 1):
            ...
            total_loss = (
                sum(fsnet_loss.values())
                + sum(recon_loss.values())
                + sum(geo_loss.values())
                + sum(prop_loss.values())
            )
            # backward
            ...
            summary = tf.compat.v1.Summary(
                value=[
                    tf.compat.v1.Summary.Value(
                        tag="lr", simple_value=optimizer.param_groups[0]["lr"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="train_loss", simple_value=total_loss
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="rot_loss_1", simple_value=fsnet_loss["Rot1"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="rot_loss_2", simple_value=fsnet_loss["Rot2"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="T_loss", simple_value=fsnet_loss["Tran"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Prop_sym_recon", simple_value=prop_loss["Prop_sym_recon"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Prop_sym_rt", simple_value=prop_loss["Prop_sym_rt"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Size_loss", simple_value=fsnet_loss["Size"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Face_loss", simple_value=recon_loss["recon_per_p"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Recon_loss_r", simple_value=recon_loss["recon_point_r"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Recon_loss_t", simple_value=recon_loss["recon_point_t"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Recon_loss_s", simple_value=recon_loss["recon_point_s"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Recon_p_f", simple_value=recon_loss["recon_p_f"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Recon_loss_se", simple_value=recon_loss["recon_point_self"]
                    ),
                    tf.compat.v1.Summary.Value(
                        tag="Face_loss_vote",
            ...
            if i % FLAGS.log_every == 0:
                logger.info(
                    "Batch {0} Loss:{1:f}, rot_loss:{2:f}, size_loss:{3:f}, trans_loss:{4:f}".format(
                        i,
                        total_loss.item(),
                        (fsnet_loss["Rot1"] + fsnet_loss["Rot2"]).item(),
                        fsnet_loss["Size"].item(),
                        fsnet_loss["Tran"].item(),
                    )
                )

        logger.info(
            ">>>>>>>>----------Epoch {:02d} train finish---------<<<<<<<<".format(epoch)
        )
