...
import evaluation
from evaluation.eval_utils import setup_logger, compute_mAP,draw_detections
...
def evaluate(argv):
    ...
    if os.path.exists(pred_result_save_path):
        ...
        for i, data in tqdm(enumerate(val_dataset, 1)):
            ...
            autoSave = True
            if autoSave :
                intrinsics = np.array([[591.0125, 0, 322.525], [0, 590.16775, 244.11084], [0, 0, 1]], dtype=np.float)
                img = detection_dict['image_path']
                img_list = img.split('/')
                img_scene = img_list[3]
                img_num = img_list[4]
                img_path = "/home/zhangyuekun/dataset/NOCS/Real/test/" + img_scene + "/"+img_num+"_color.png"
                # print(img_path)
                import cv2
                img_oral = cv2.imread(img_path)
                print("img_path",img_path)
                print("gt_RTs",detection_dict['gt_RTs'])
                draw_detections(img_oral,"/home/zhangyuekun/GPV_Pose/result/pic_vote_box","real_test",
                                img_scene,img_num,intrinsics,detection_dict['pred_RTs'],detection_dict['pred_scales'],
                                detection_dict['pred_class_ids'],
                                detection_dict['gt_RTs'],detection_dict['gt_scales'],detection_dict['gt_class_ids'])
