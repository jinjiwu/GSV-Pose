...
from evaluation.eval_utils_v1 import (
    get_3d_bbox,
...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode="test", n_pts=1024, img_size=256):
        """
        ...
        if (
            self.per_obj in self.cat_names
            and self.per_obj != "can"
            ...
            if os.path.exists(img_list_cache_filename):
                ...
                img_list_obj = [
                    line.rstrip("\n")
                    for line in open(
                        os.path.join(self.data_dir, img_list_cache_filename)
                    )
                ]
            else:
    ...
    def __getitem__(self, index):
        ...
        if img_type == "real":
            ...
            detection_file = os.path.join(
                self.detection_dir, dataset_split, f"results_test_{scene}_{img_id}.pkl"
            )
        else:
            dataset_split = "CAMERA25"
            detection_file = os.path.join(
                self.detection_dir, dataset_split, f"results_val_{scene}_{img_id}.pkl"
            )
        ...
        for j in range(num_instance):
            ...
            cv2.imwrite(
                f"eval_logs/roi_dataset_{index}.png", roi_img.transpose(1, 2, 0)
            )
        ...
        data_dict["depth_normalize"] = torch.as_tensor(
            roi_depth_norms.astype(np.float32)
        ).contiguous()
        data_dict["sym_info"] = torch.as_tensor(
            sym_infos.astype(np.float32)
        ).contiguous()
        data_dict["mean_shape"] = torch.as_tensor(
            mean_shapes, dtype=torch.float32
        ).contiguous()
        data_dict["roi_coord_2d"] = torch.as_tensor(
            roi_coord_2ds, dtype=torch.float32
        ).contiguous()
