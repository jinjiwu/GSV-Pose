...
class PoseDataset(data.Dataset):
    def __init__(
        ...
        img_list_path = [
            "CAMERA/train_list.txt",
            "Real/train_list.txt",
            "CAMERA/val_list.txt",
            "Real/test_list.txt",
        ]
        model_file_path = [
            "obj_models/camera_train.pkl",
            "obj_models/real_train.pkl",
            "obj_models/camera_val.pkl",
        ...
        for path in img_list_path:
            img_list += [
                os.path.join(path.split("/")[0], line.rstrip("\n"))
        ...
        self.invaild_list = []
        self.mug_sym = mmcv.load(
            os.path.join(self.data_dir, "Real/train/mug_handle.pkl")
        )
    ...
    def __getitem__(self, index):
        ...
        depth_normalize = (roi_depth - np.min(depth_v_value)) / (
            np.max(depth_v_value) - np.min(depth_v_value)
        )
        ...
        fsnet_scale, mean_shape = self.get_fs_net_scale(
            self.id2cat_name[str(cat_id + 1)], model, nocs_scale
        )
        ...
        sym_info = self.get_sym_info(
            self.id2cat_name[str(cat_id + 1)], mug_handle=mug_handle
        )
        # add nnoise to roi_mask
        roi_mask_def = defor_2D(
            roi_mask, rand_r=FLAGS.roi_mask_r, rand_pro=FLAGS.roi_mask_pro
        )
        ...
        data_dict["roi_depth"] = torch.as_tensor(
            roi_depth.astype(np.float32)
        ).contiguous()
        data_dict["dense_depth"] = torch.as_tensor(
            dense_depth.astype(np.float32)
        ).contiguous()
        data_dict["depth_normalize"] = torch.as_tensor(
            depth_normalize.astype(np.float32)
        ).contiguous()
        ...
        data_dict["fsnet_scale"] = torch.as_tensor(
            fsnet_scale, dtype=torch.float32
        ).contiguous()
        data_dict["sym_info"] = torch.as_tensor(
            sym_info.astype(np.float32)
        ).contiguous()
        data_dict["roi_coord_2d"] = torch.as_tensor(
            roi_coord_2d, dtype=torch.float32
        ).contiguous()
        data_dict["mean_shape"] = torch.as_tensor(
            mean_shape, dtype=torch.float32
        ).contiguous()
        data_dict["aug_bb"] = torch.as_tensor(bb_aug, dtype=torch.float32).contiguous()
        data_dict["aug_rt_t"] = torch.as_tensor(
            rt_aug_t, dtype=torch.float32
        ).contiguous()
        data_dict["aug_rt_R"] = torch.as_tensor(
            rt_aug_R, dtype=torch.float32
        ).contiguous()
        data_dict["roi_mask_deform"] = torch.as_tensor(
            roi_mask_def, dtype=torch.float32
        ).contiguous()
        data_dict["model_point"] = torch.as_tensor(
            model, dtype=torch.float32
        ).contiguous()
        data_dict["nocs_scale"] = torch.as_tensor(
            nocs_scale, dtype=torch.float32
        ).contiguous()

        return data_dict

    def generate_aug_parameters(
        self, s_x=(0.8, 1.2), s_y=(0.8, 1.2), s_z=(0.8, 1.2), ax=50, ay=50, az=50, a=15
    ):
        # for bb aug
        ...
        # for R, t aug
        Rm = get_rotation(
            np.random.uniform(-a, a), np.random.uniform(-a, a), np.random.uniform(-a, a)
        )
        dx = np.random.rand() * 2 * ax - ax
        dy = np.random.rand() * 2 * ay - ay
        dz = np.random.rand() * 2 * az - az
        return (
            np.array([ex, ey, ez], dtype=np.float32),
            np.array([dx, dy, dz], dtype=np.float32) / 1000.0,
            Rm,
        )
    ...
    def get_fs_net_scale(self, c, model, nocs_scale):
        ...
        # scale residual
        return np.array([lx_t - unitx, ly_t - unity, lz_t - unitz]), np.array(
            [unitx, unity, unitz]
        )
