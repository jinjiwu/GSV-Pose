...
class recon_6face_loss(nn.Module):
    ...
    def forward(self, name_list, pred_list, gt_list, sym, obj_ids, save_path=None):
        ...
        if 'Point_voting' in name_list:
            ...
            with torch.no_grad():
                wgter = weighter()
                weight = wgter.get_weight(gt_list['Points'],obj_ids)
            # with torch.no_grad():
            ...
            recon_point_vote, recon_point_r, recon_point_t, recon_point_s, recon_point_self = self.cal_recon_loss_vote(
                                                                             ...
                                                                             gt_list['Size'],
                                                                             gt_list['Mean_shape'],
                                                                             sym, obj_ids, save_path,weight=weight)
    ...
    def cal_recon_loss_vote(self, pc, face_normal, face_dis, face_c, p_rot_g, f_rot_g, p_rot_r, f_rot_r, p_t, p_s,
                            gt_R, gt_t, gt_s, mean_shape, sym, obj_ids, save_path=None,weight=None):
        res_vote = 0.0
        res_recon_geo_r = 0.0
        res_recon_geo_t = 0.0
        res_recon_geo_s = 0.0
        ...
        for i in range(bs):
            ...
            import numpy as np
            weight_i = np.tile(weight[i], (6, 1)).T
            f_c_now *= torch.tensor(weight_i, device=f_c_now.device, dtype=f_c_now.dtype)
