...
from network.weight_net.weight_net import weighter
import numpy as np
...
class recon_6face_loss(nn.Module):
    ...
    def forward(self, name_list, pred_list, gt_list, sym, obj_ids, save_path=None):
        ...
        if "Per_point" in name_list:
            ...
            loss_list["recon_per_p"] = (
                FLAGS.recon_n_w * res_normal + FLAGS.recon_d_w * res_dis
            )
            loss_list["recon_p_f"] = FLAGS.recon_f_w * res_f
        if "Point_voting" in name_list:
            F_c_detach = pred_list["F_c"].clone().detach()
            # # 处理权重
            # torch.autograd.set_detect_anomaly(True)
            with torch.no_grad():
                timer_epoch.start("weight")
                weight = self.weighter.get_weight(gt_list["Points"], obj_ids)
                timer_epoch.stop("weight")
            # with torch.no_grad():
            #     print("face_c",F_c_detach)
            #     for i in range(len(gt_list['Points'])):
            #         weight_i = np.tile(weight[i], (6, 1)).T
            #         # weight_i = weight[i].repeat(6,1).t()
            #         F_c_detach_i = F_c_detach[i].clone().detach()
            #         F_c_detach_i *= torch.tensor(weight_i, device=F_c_detach.device, dtype=F_c_detach.dtype)
            #         F_c_detach[i] = F_c_detach_i
            #     print("after_face_c",F_c_detach)
            # F_c_detach = torch.from_numpy(F_c_detach)
            (
                recon_point_vote,
                recon_point_r,
                recon_point_t,
        ...
        if "Point_sampling" in name_list:
            loss_list["recon_point_sample"] = (
                FLAGS.recon_s_w
                * self.cal_recon_loss_sample(pred_list["Pc_sk"], pred_list["F_c"])
            )

        if "Point_c_reg" in name_list:
            loss_list["recon_point_c_reg"] = (
                FLAGS.recon_c_w * self.cal_recon_loss_direct(pred_list["F_c"])
            )
        return loss_list
    ...
    def cal_recon_loss_vote(
        ...
        for i in range(bs):
            ...
            weight_i = np.tile(weight[i], (6, 1)).T
            f_c_now *= torch.tensor(
                weight_i, device=f_c_now.device, dtype=f_c_now.dtype
            )
            ...
            if sym[i, 0] == 0:
                ...
                # cal gt
                dn_gt = gt_r_x * (
                    -(torch.dot(gt_r_x, gt_t_now + gt_r_x * re_s_now[0] / 2))
                )
                ...
                if save_path is not None:
                    ...
                    plane_parameter = plane_parameter.detach().cpu().numpy()
                    np.savetxt(
                        save_path + f"_{i}_plane_parameter_xp.txt", plane_parameter
                    )
                # z +
                ...
                # cal gt
                dn_gt = gt_r_z * (
                    -(torch.dot(gt_r_z, gt_t_now + gt_r_z * re_s_now[2] / 2))
                )
                # adjust the sign of dn_gt
                ...
                if save_path is not None:
                    ...
                    plane_parameter = plane_parameter.detach().cpu().numpy()
                    np.savetxt(
                        save_path + f"_{i}_plane_parameter_zp.txt", plane_parameter
                    )
            ...
            # cal gt
            dn_gt = -gt_r_y * (
                -(torch.dot(-gt_r_y, gt_t_now - gt_r_y * re_s_now[1] / 2))
            )
            # adjust the sign of dn_gt
            ...
            new_y, new_x = get_vertical_rot_vec(
                f_rot_g[i], f_rot_r[i], p_rot_g[i, ...], p_rot_r[i, ...]
            )
    ...
    def cal_recon_loss_point(
        ...
        re_s = gt_s + mean_shape
        pc_proj = torch.bmm(
            gt_R.permute(0, 2, 1), (pc.permute(0, 2, 1) - gt_t.view(bs, 3, 1))
        ).permute(0, 2, 1)
        # stack gt
        for i in range(bs):
            ...
            c_y_up = face_f[i, :, 0]
            cc_y_up = torch.norm(
                f_n_yplus * f_d_yplus.view(-1, 1)
                - gt_r_y.view(1, 3).repeat(f_n_yplus.shape[0], 1)
                * f_d_gt_yplus.view(-1, 1),
                dim=1,
            )
            ...
            if sym[i, 0] == 0:
                if obj_id != 5:
                    ...
                    cc_x_up = torch.norm(
                        ...
                        - gt_r_x.view(1, 3).repeat(f_n_xplus.shape[0], 1)
                        * f_d_gt_xplus.view(-1, 1),
                        dim=1,
                    )
                    ...
                    cc_x_down = torch.norm(
                        ...
                        - (-gt_r_x).view(1, 3).repeat(f_n_xminus.shape[0], 1)
                        * f_d_gt_xminus.view(-1, 1),
                        dim=1,
                    )
                ...
                cc_z_up = torch.norm(
                    ...
                    - gt_r_z.view(1, 3).repeat(f_n_zplus.shape[0], 1)
                    * f_d_gt_zplus.view(-1, 1),
                    dim=1,
                )
                ...
                cc_z_down = torch.norm(
                    ...
                    - (-gt_r_z).view(1, 3).repeat(f_n_zminus.shape[0], 1)
                    * f_d_gt_zminus.view(-1, 1),
                    dim=1,
                )
            ...
            cc_y_down = torch.norm(
                ...
                - (-gt_r_y).view(1, 3).repeat(f_n_yminus.shape[0], 1)
                * f_d_gt_yminus.view(-1, 1),
                dim=1,
            )
