...
def gen_data(ref_points, src_points):
    ...
    all_points.points = o3d.utility.Vector3dVector(
        np.concatenate((ref_points, src_points), axis=0)
    )

    # 法线估计
    radius = 0.02  # 搜索半径
    max_nn = 60  # 邻域内用于估算法线的最大点数
    all_points.estimate_normals(
    ...
    all_data.append(
        dict(
            ...
            normals=np.asarray(all_normals),
            label=27,
        )
    )
    all_data = np.asarray(all_data)
    dump_pickle(np.asarray(all_data), "network/weight_net/data/test.pkl")
