...
from tools.plane_utils import random_crop_point_cloud_with_plane
...
class GPVPose(nn.Module):
    ...
    def forward(self, depth, obj_id, camK,
        ...
        if FLAGS.train:
            ...
            # 随机剪裁
            # PC_crop = []
            # for i in range(len(PC_da)):
            #     PC_crop.append(random_crop_point_cloud_with_plane(points=PC_da[i].cpu(),keep_ratio=0.7))

            # PC_crop = torch.tensor([item.cpu().detach().numpy() for item in PC_crop]).cuda()
            # PC = PC_crop
